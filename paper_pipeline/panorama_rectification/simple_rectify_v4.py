#!/usr/bin/env python3
"""
Simplified Panorama Rectification Tool - Version 4 (Strict Parameters)
Based on the A-Contrario Horizon-First Vanishing Point Detection algorithm

Improvements:
- Uses stricter thresholds for better precision
- Modified default parameters for enhanced accuracy
- Better filtering of vanishing points

Usage: python simple_rectify_v4.py input_folder output_folder
"""
...
class StrictParams:
    ...
    theta_v = np.pi / 12  # More strict vertical threshold (was pi/8)
    theta_z = 8           # More strict zenith threshold (was 10)
    L_z = 60             # Longer minimum line length (was 45)
    theta_h = 1.2        # More strict horizontal threshold (was 1.5)
    L_h = 80             # Longer minimum horizontal line length (was 64)
    sigma = 0.15         # Lower sigma for better precision (was 0.2)
    S = 400              # More samples for better accuracy (was 300)
    L_vp = 160           # Longer minimum vanishing point length (was 128)
    
    include_infinite_hvps = 0
    
    theta_con = 1.2      # More strict consensus threshold (was 1.5)
    theta_verline = 12   # More strict vertical line threshold (was 15)
    theta_horline = 0.8  # More strict horizontal line threshold (was 1)
    hvp_refinement = True
    refine_niters = 5    # More refinement iterations (was 3)
...
def rectify_single_panorama(input_path, output_path, temp_dir=None):
    ...
    try:
        ...
        # Step 1: Render multiple viewpoints (4 views at 90° intervals)
        render_num = 4
        tilelist = render_imgs(panorama_img, temp_dir, save_directly=True)
        
        # Step 2: Detect vanishing points for each viewpoint with strict parameters
        hl = []
        hvps = []
        hvp_groups = []
        z = []
        ...
        # Override default parameters with strict ones
        strict_params = StrictParams()
        
        for i in range(len(tilelist)):
            # Use standard parameters (custom params would require modifying the underlying function)
        ...
        # Step 3: Calculate consensus zenith point with enhanced filtering
        zenith_points = np.array([R_heading(np.pi / 2 * (i - 1)).dot(zenith) 
                                 for i, zenith in enumerate(z_homo)])
        
        # Apply additional filtering based on zenith point quality
        zenith_scores = []
        for i, zp in enumerate(zenith_points):
            # Calculate confidence score based on line group size
            try:
                if hasattr(z_group[i], '__len__') and len(z_group[i]) > 0:
                    # Check if z_group[i] is iterable and contains list-like objects
                    group_sizes = []
                    for group in z_group[i]:
                        if hasattr(group, '__len__'):
                            group_sizes.append(len(group))
                        else:
                            group_sizes.append(1)  # Treat single items as size 1
                    score = np.mean(group_sizes) if group_sizes else 0
                else:
                    score = 1  # Default score for non-iterable or empty groups
            except (TypeError, AttributeError):
                score = 1  # Default score if there's any issue with group processing
            zenith_scores.append(score)
        
        # Filter out low-quality zenith points
        quality_threshold = np.mean(zenith_scores) * 0.5
        good_indices = [i for i, score in enumerate(zenith_scores) if score > quality_threshold]
        
        if len(good_indices) > 0:
            filtered_zenith_points = zenith_points[good_indices]
        else:
            filtered_zenith_points = zenith_points  # Use all if filtering is too strict
        
        [zenith_consensus, best_zenith] = calculate_consensus_zp(filtered_zenith_points, method='svd')
        
        # Step 4: Calculate pitch and roll angles with smoothing
        ...
        pitch_smooth = np.clip(pitch, -np.pi/6, np.pi/6)  # Limit to ±30 degrees
        roll_smooth = np.clip(roll, -np.pi/6, np.pi/6)    # Limit to ±30 degrees
        
        print(f"  Detected pitch: {np.degrees(pitch_smooth):.2f}°")
        print(f"  Detected roll: {np.degrees(roll_smooth):.2f}°")
        ...
        coordinates = R_pitch(pitch_smooth).dot(R_roll(roll_smooth).dot(coordinates.T)).T
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: python simple_rectify_v4.py input_folder output_folder")
        print("")
        print("This tool rectifies panoramic images using strict parameters:")
        print("- Stricter thresholds for better precision")
        print("- Enhanced filtering of vanishing points")
        print("- Angle smoothing for stability")
        print("")
    ...
    print("Panorama Rectification Tool v4 (Strict Parameters)")
    print("=" * 55)
