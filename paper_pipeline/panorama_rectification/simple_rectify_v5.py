#!/usr/bin/env python3
"""
Simplified Panorama Rectification Tool - Version 5 (High Resolution)
Based on the A-Contrario Horizon-First Vanishing Point Detection algorithm

Improvements:
- Enhanced coordinate grid resolution for better quality
- Optimized interpolation with prefiltering
- Better edge handling and boundary conditions
- Adaptive processing based on image size

Usage: python simple_rectify_v5.py input_folder output_folder
"""
...
def rectify_single_panorama(input_path, output_path, temp_dir=None):
    ...
    try:
        ...
        # Step 5: Create rectified panorama with enhanced resolution
        height = im.height
        width = im.width
        
        # Adaptive resolution scaling based on image size
        if height * width > 4000000:  # For large images (>4MP)
            resolution_factor = 1.5
        else:
            resolution_factor = 2.0
        
        # Generate higher resolution coordinate grid
        dense_height = int(height * resolution_factor)
        dense_width = int(width * resolution_factor)
        
        u, v = np.mgrid[-np.pi/2:np.pi/2:dense_height*1j, -np.pi:np.pi:dense_width*1j]
        ...
        coordinates = coordinates.reshape(2, dense_height, dense_width)
        
        # Apply coordinate mapping to rectify the image with enhanced interpolation
        img = skimage.io.imread(input_path)
        
        # Pre-filter the image to reduce aliasing
        img_filtered = np.dstack([
            gaussian_filter(img[:, :, 0], sigma=0.5),
            gaussian_filter(img[:, :, 1], sigma=0.5),
            gaussian_filter(img[:, :, 2], sigma=0.5)
        ])
        
        # High-quality rectification with spline interpolation
        dense_rectified = np.dstack([
            map_coordinates(img_filtered[:, :, 0], coordinates, order=2, mode='wrap', prefilter=True),
            map_coordinates(img_filtered[:, :, 1], coordinates, order=2, mode='wrap', prefilter=True),
            map_coordinates(img_filtered[:, :, 2], coordinates, order=2, mode='wrap', prefilter=True)
        ])
        
        # Resize back to original dimensions with anti-aliasing
        from PIL import Image as PILImage
        dense_pil = PILImage.fromarray(dense_rectified.astype(np.uint8))
        rectified_pil = dense_pil.resize((width, height), PILImage.LANCZOS)
        rectified_img = np.array(rectified_pil)
        
        # Post-processing: slight sharpening to compensate for interpolation blur
        from scipy.ndimage import laplace
        sharpening_strength = 0.3
        for channel in range(3):
            laplacian = laplace(rectified_img[:, :, channel].astype(np.float64))
            rectified_img[:, :, channel] = np.clip(
                rectified_img[:, :, channel] - sharpening_strength * laplacian,
                0, 255
            ).astype(np.uint8)
        
        # Save rectified image
        skimage.io.imsave(output_path, rectified_img)
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: python simple_rectify_v5.py input_folder output_folder")
        print("")
        print("This tool rectifies panoramic images using high-resolution processing:")
        print("- Enhanced coordinate grid resolution (1.5x-2x)")
        print("- Quadratic spline interpolation with prefiltering")
        print("- Adaptive processing based on image size")
    ...
    print("Panorama Rectification Tool v5 (High Resolution)")
    print("=" * 55)
