#!/usr/bin/env python3
"""
Simplified Panorama Rectification Tool - Version 3 (Higher Order Interpolation)
Based on the A-Contrario Horizon-First Vanishing Point Detection algorithm

Improvements:
- Uses cubic spline interpolation (order=3) for smoother image quality
- Better image quality but slightly slower processing

Usage: python simple_rectify_v3.py input_folder output_folder
"""
...
def rectify_single_panorama(input_path, output_path, temp_dir=None):
    ...
    try:
        ...
        # Apply coordinate mapping to rectify the image with cubic spline interpolation
        img = skimage.io.imread(input_path)
        rectified_img = np.dstack([
            map_coordinates(img[:, :, 0], coordinates, order=3, mode='reflect'),
            map_coordinates(img[:, :, 1], coordinates, order=3, mode='reflect'),
            map_coordinates(img[:, :, 2], coordinates, order=3, mode='reflect')
        ])
...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: python simple_rectify_v3.py input_folder output_folder")
        print("")
        print("This tool rectifies panoramic images using enhanced parameters:")
        print("- Cubic spline interpolation (order=3) for smoother image quality")
        print("- Reflect mode for better edge handling")
        print("")
    ...
    print("Panorama Rectification Tool v3 (High Quality Interpolation)")
    print("=" * 60)
